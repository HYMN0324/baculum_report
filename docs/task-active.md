...
**체크리스트**:
- [ ] `<thead>` 섹션의 `<th>` 순서 변경 완료
- [ ] `<tbody>` 섹션의 `<td>` 순서 변경 완료
- [ ] 테스트 리포트 생성하여 컬럼 순서 정상 확인
- [ ] 모든 기존 데이터 필드가 정상적으로 표시되는지 확인
...
**체크리스트**:
- [ ] `.env.example`에 `BACULUM_WEB_HOST`, `BACULUM_WEB_PORT` 추가
- [ ] `Config` 클래스에 `baculum_web_host`, `baculum_web_port`, `has_baculum_web_config()` 구현
- [ ] `ReportGenerator.generate_html_report()`에서 `baculum_web_url` 전달
- [ ] HTML 템플릿에 "상세보기" 컬럼 추가
- [ ] 돋보기 아이콘 링크 구현 (조건부 렌더링)
- [ ] `.env` 파일에 실제 Baculum 웹 설정 추가하여 테스트
- [ ] 링크 클릭 시 올바른 Baculum 페이지로 이동하는지 확인
- [ ] Baculum 웹 설정이 없을 때 "-" 표시 확인
- [ ] 이메일 클라이언트에서도 링크가 정상 작동하는지 확인
...
**체크리스트**:
- [ ] 실패한 백업 상세 섹션을 테이블 구조로 변경
- [ ] 테이블 컬럼에 "실패 시간" 명칭 사용 (기존 "완료 시간"에서 변경)
- [ ] 에러 로그 표시 방법 구현 (토글 또는 별도 영역)
- [ ] 테이블 행 배경색을 연한 빨간색으로 설정
- [ ] 에러 로그가 없는 경우 적절한 메시지 표시
- [ ] 테스트 리포트 생성하여 레이아웃 정상 확인
- [ ] 이메일 클라이언트에서 테이블이 정상적으로 렌더링되는지 확인
- [ ] 에러 로그 토글 기능이 이메일에서도 작동하는지 확인 (CSS 기반인 경우)
