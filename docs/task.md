...
### 2.1 API 클라이언트 구현
- [ ] Bacula REST API 문서 검토 (https://bacula.org/downloads/baculum/baculum-api-v1)
- [ ] `test.py` 코드 분석 - API 호출 방식, 인증 방법, 응답 구조 파악
- [ ] `src/api/__init__.py` 생성
- [ ] `src/api/bacula_client.py` 생성
  - BaculaClient 클래스 설계
  - `__init__`: API URL, 인증 정보 초기화
  - `connect()`: API 연결 테스트
  - `get_jobs()`: 백업 작업 목록 조회
  - `_request()`: 공통 HTTP 요청 처리 (타임아웃, 재시도)
- [ ] `.env`에서 설정 로드 (python-dotenv 사용)
  ```python
  ...
  ```
- [ ] 타임아웃 설정 (requests timeout=10)
- [ ] 재시도 로직 추가 (최대 3회, 지수 백오프)
- [ ] 커스텀 예외 클래스 정의 (BaculaAPIError, ConnectionError, TimeoutError)
- [ ] 상세 로깅 추가 (요청/응답, 에러 정보)
...
- [ ] API 응답 데이터 파싱 함수 구현
- [ ] 백업 상태 분류 로직 (성공/실패/진행중)
- [ ] **실패한 백업의 에러 로그 추출** (필수 - 강조 표시)

### 2.3 API 테스트
- [ ] **실제 API 호출 테스트** (실제 Bacula 시스템 대상)
  - API 연결 확인
  - 응답 데이터 구조 파악 및 저장 (JSON 샘플)
  - API 호출 시간 측정 (10초 이하 확인)
- [ ] 응답 데이터 기반으로 파싱 로직 구현
- [ ] 파싱 로직 단위 테스트 작성 (실제 응답 샘플 사용)
